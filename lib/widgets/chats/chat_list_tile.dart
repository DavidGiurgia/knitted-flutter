...
class ChatListTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final chatRoomsProvider = Provider.of<ChatRoomsProvider>(
      context,
    ); // Folosește ChatRoomsProvider
    final currentUser = userProvider.user;
    final lastMessage =
        chatRoomsProvider.lastMessages[room
            .id]; // Ia ultimul mesaj din provider
    ...
    final participants =
        chatRoomsProvider.roomParticipants[room.id] ??
        []; // Ia participanții din provider
    ...
    return ListTile(
      ...
      trailing:
          (room.type == "temporary") ? _buildCountdown(room.expiresAt!) : null,
      onTap:
          () => Navigator.push(
            context,
            MaterialPageRoute(
